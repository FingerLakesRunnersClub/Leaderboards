...
public sealed class OverallResultsCalculator(ICommunityStarCalculator starCalculator) : IOverallResultsCalculator
{
	...
	public RankedList<Date, Result> CompletedPersonal(Iteration iteration, Filter filter = null)
		=> RankedList(iteration, iteration.AllCourses.SelectMany(c => c.Results.For(iteration).Earliest(filter)).GroupBy(r => r.Result.Athlete).Where(a => a.Key.Challenge(iteration) != iteration.OfficialChallenge && (a.Key.Challenge(iteration)?.Courses.All(c => a.Any(r => r.Result.CourseID == c.ID)) ?? false)), g => g.Max(r => r.Value), g => Start(iteration).Subtract(g.Max(r => r.Value)?.Value ?? Start(iteration)), g => (uint)g.Count());
