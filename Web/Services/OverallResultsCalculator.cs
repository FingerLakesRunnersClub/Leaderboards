...
public sealed class OverallResultsCalculator
{
	...
	public RankedList<Date, Result> Completed(Filter filter = null)
		=> RankedList(_officialCourses.SelectMany(c => c.Results.For(_iteration).Earliest(filter)).GroupBy(r => r.Result.Athlete).Where(a => a.Key.Challenge(_iteration) == _iteration.OfficialChallenge && a.Count() == _officialCourses.Length), g => g.Max(r => r.Value), g => _start.Subtract(g.Max(r => r.Value)?.Value ?? _start), g => (uint)g.Count());
	...
	public RankedList<Stars, Result> Community(Filter filter = null)
		=> RankedList(_courses.SelectMany(c => c.Results.For(_iteration).CommunityStars(_starCalculator, filter)).GroupBy(g => g.Result.Athlete), g => new Stars((ushort)g.Sum(r => r.Value.Value)), g => g.Sum(s => s.Value.Value), g => (uint)g.Count());
	...
	public RankedList<TeamResults, Result> TeamPoints(Filter filter = null)
		=> _officialCourses.SelectMany(c => c.Results.For(_iteration).TeamPoints(_iteration, filter))
			.GroupBy(r => r.Value.Team)
			.Select(g => new TeamResults
			{
				Team = g.Key,
				AgeGradePoints = (byte)(g.Sum(r => r.Value.AgeGradePoints) + Team.Teams.Count * (_officialCourses.Length - g.Count())),
				MostRunsPoints = (byte)(g.Sum(r => r.Value.MostRunsPoints) + Team.Teams.Count * (_officialCourses.Length - g.Count()))
			})
			.Rank();
	...
	private RankedList<TeamMember, Result> RankTeam(Ranked<Time, Result>[] results)
	{
		...
		for (byte rank = 1; rank <= ranked.Length; rank++)
		{
			...
			ranks.Add(new Ranked<TeamMember, Result>
			{
				...
				Result = new Result { Athlete = value.Athlete, StartTime = _iteration.StartDate?.ToDateTime(TimeOnly.MinValue) ?? DateTime.Now },
				Count = value.Courses,
