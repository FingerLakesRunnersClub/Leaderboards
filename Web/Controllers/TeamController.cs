...
public sealed class TeamController(IIterationManager iterationManager, IOverallResultsCalculator overall) : Controller
{
	[HttpGet]
	...
	private async Task<ViewModel<TeamMembers>> GetMembers(Team team)
	{
		...
		var results = new TeamMembers
		{
			Team = team,
			ResultType = "Members",
			RankedResults = overall.TeamMembers(iteration, team)
		};
