...
public class OverallResults
{
	...
	public RankedList<Points> CommunityStars(Category category = null)
		=> RankedList(_courses.SelectMany(c => c.CommunityStars(category).Where(p => p.Value.Value > 0)).GroupBy(r => r.Result.Athlete), g => new Points(g.Sum(c => c.Value.Value), 0), g => g.Sum(c => c.Value.Value));
