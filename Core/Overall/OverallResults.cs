...
public class OverallResults
{
	...
	public RankedList<Points> CommunityPoints(Category category = null)
		=> RankedList(_courses.SelectMany(c => c.CommunityPoints(category).Where(p => p.Value.Value > 0)).GroupBy(r => r.Result.Athlete), g => new Points(g.Sum(c => c.Value.Value), 0), g => g.Sum(c => c.Value.Value));
