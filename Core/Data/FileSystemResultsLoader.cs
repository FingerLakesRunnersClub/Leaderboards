...
public sealed class FileSystemResultsLoader : IFileSystemResultsLoader
{
	...
	private IDictionary<uint, Race> _cachedRaces;
	...
	public Race[] GetRaces()
	{
		...
		_config.CourseNames = _cachedRaces
			.OrderBy(r => r.Value.Courses[0].Distance?.Meters ?? Distance.MetersPerMarathon)
			.ToDictionary(r => r.Key, r => r.Value.Name);

		return _cachedRaces.Values.ToArray();
	}
	...
	private IDictionary<uint, Course> _allResults;

	public async Task<IDictionary<uint, Course>> GetAllResults()
	{
		if (_allResults is not null)
			return _allResults;

		var paths = _cachedRaces.Select(r => Path.Combine(_config.FileSystemResults, r.Value.Date.ToString("yyyy-MM-dd"), $"{r.Value.Name}.txt"));
		var tasks = paths
		...
		return _allResults = events.ToDictionary(g => g.First().ID, g => new Course { ID = g.First().ID, Race = g.First().Race, Results = g.SelectMany(r => r.Results).OrderBy(r => r.Duration).ToArray() });
	}
	...
	private Course GetCourse(string file)
	{
		...
		var race = _cachedRaces[name.GetID()];
		var course = new Course
		{
			ID = race.ID,
			Race = race
